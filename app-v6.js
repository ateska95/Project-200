...
function dailyWeightSeries() {
  ...
  return [...byDate.values()].sort((a, b) => a.date.localeCompare(b.date) || a.createdAt - b.createdAt);
}

function rollingWeightSeries(windowSize = 3) {
  const daily = dailyWeightSeries();
  return daily.map((entry, index) => {
    const window = daily.slice(Math.max(0, index - windowSize + 1), index + 1);
    const trendWeight = window.reduce((sum, item) => sum + item.weight, 0) / window.length;
    return {
      ...entry,
      trendWeight,
...
function latestTrendPoint() {
  return rollingWeightSeries(3).at(-1) || null;
}

function filterTrendRange(series, range) {
  if (!series.length || range === 'all') return series;
  const days = Number(range);
  if (!Number.isFinite(days)) return series;
  const anchor = series.at(-1).date;
  const cutoff = addDays(anchor, -(days - 1));
  return series.filter(entry => entry.date >= cutoff);
}

function chartDateLabel(value, includeYear = false) {
  return parseLocalDate(value).toLocaleDateString(undefined, includeYear
    ? { month: 'short', day: 'numeric', year: 'numeric' }
    : { month: 'short', day: 'numeric' });
}

function trendChartGeometry(series, width, height, compact) {
  const margin = compact
    ? { top: 8, right: 8, bottom: 8, left: 8 }
    : { top: 30, right: 32, bottom: 46, left: 58 };
  const chartWidth = width - margin.left - margin.right;
  const chartHeight = height - margin.top - margin.bottom;
  const minTime = parseLocalDate(series[0].date).getTime();
  const rawMaxTime = parseLocalDate(series.at(-1).date).getTime();
  const maxTime = rawMaxTime === minTime ? minTime + 86400000 : rawMaxTime;
  const values = series.flatMap(entry => [entry.weight, entry.trendWeight]);
  const minValue = Math.min(...values);
  const maxValue = Math.max(...values);
  const padding = compact ? Math.max(0.8, (maxValue - minValue) * 0.28) : Math.max(1.4, (maxValue - minValue) * 0.2);
  const minWeight = minValue - padding;
  const maxWeight = maxValue + padding;
  const weightRange = Math.max(1, maxWeight - minWeight);
  const x = date => margin.left + ((parseLocalDate(date).getTime() - minTime) / (maxTime - minTime)) * chartWidth;
  const y = weight => margin.top + ((maxWeight - weight) / weightRange) * chartHeight;
  return { margin, chartWidth, chartHeight, minTime, maxTime, minWeight, maxWeight, x, y };
}

function renderCompactTrendChart(series) {
  const svg = $('dashboard-trend-chart');
  const empty = $('dashboard-trend-empty');
  const display = filterTrendRange(series, '30');
  const hasEntries = display.length > 0;
  svg.classList.toggle('hidden', !hasEntries);
  ...
  const width = 640;
  const height = 90;
  const { x, y, margin, chartHeight } = trendChartGeometry(display, width, height, true);
  const trendPoints = display.map(entry => `${x(entry.date).toFixed(1)},${y(entry.trendWeight).toFixed(1)}`).join(' ');
  const areaPoints = `${x(display[0].date).toFixed(1)},${(margin.top + chartHeight).toFixed(1)} ${trendPoints} ${x(display.at(-1).date).toFixed(1)},${(margin.top + chartHeight).toFixed(1)}`;
  const dots = display.map(entry => `<circle class="trend-raw-point compact" cx="${x(entry.date)}" cy="${y(entry.weight)}" r="2.6"><title>${formatDate(entry.date)}: ${entry.weight.toFixed(1)} lb</title></circle>`).join('');

  svg.innerHTML = `<polygon class="trend-chart-area" points="${areaPoints}"></polygon>
    ${dots}
    <polyline class="trend-average-line compact" points="${trendPoints}"></polyline>`;
}

function renderExpandedTrendChart(series) {
  const svg = $('expanded-weight-trend-chart');
  const empty = $('expanded-trend-empty');
  const wrap = $('expanded-trend-chart-wrap');
  const display = filterTrendRange(series, weightTrendRange);
  const hasEntries = display.length > 0;
  empty.classList.toggle('hidden', hasEntries);
  ...
  const width = 900;
  const height = 420;
  const geometry = trendChartGeometry(display, width, height, false);
  const { margin, chartWidth, chartHeight, minWeight, maxWeight, x, y } = geometry;
  const trendPoints = display.map(entry => `${x(entry.date).toFixed(1)},${y(entry.trendWeight).toFixed(1)}`).join(' ');
  const areaPoints = `${x(display[0].date).toFixed(1)},${(margin.top + chartHeight).toFixed(1)} ${trendPoints} ${x(display.at(-1).date).toFixed(1)},${(margin.top + chartHeight).toFixed(1)}`;
  const rawPoints = display.map(entry => `<circle class="trend-raw-point" cx="${x(entry.date)}" cy="${y(entry.weight)}" r="5"><title>${formatDate(entry.date)}: ${entry.weight.toFixed(1)} lb actual; ${entry.trendWeight.toFixed(1)} lb trend</title></circle>`).join('');
  const gridTicks = 4;
  const grid = Array.from({ length: gridTicks + 1 }, (_, index) => {
    const value = maxWeight - ((maxWeight - minWeight) * index / gridTicks);
    const py = y(value);
    return `<line class="trend-grid-line" x1="${margin.left}" y1="${py}" x2="${width - margin.right}" y2="${py}"></line>
      <text class="trend-axis-label" x="${margin.left - 10}" y="${py + 4}" text-anchor="end">${value.toFixed(1)}</text>`;
  }).join('');
  const first = display[0];
  const last = display.at(-1);
  const dateLabels = `<text class="trend-axis-label" x="${margin.left}" y="${height - 15}" text-anchor="start">${chartDateLabel(first.date, weightTrendRange === 'all')}</text>
    <text class="trend-axis-label" x="${width - margin.right}" y="${height - 15}" text-anchor="end">${chartDateLabel(last.date, weightTrendRange === 'all')}</text>`;
  const goal = state.settings.goalWeight;
  const goalInView = goal >= minWeight && goal <= maxWeight;
  const goalLine = goalInView
    ? `<line class="trend-goal-line" x1="${margin.left}" y1="${y(goal)}" x2="${width - margin.right}" y2="${y(goal)}"></line>
       <text class="trend-axis-label trend-goal-label" x="${width - margin.right}" y="${y(goal) - 8}" text-anchor="end">Goal ${goal} lb</text>`
    : `<text class="trend-axis-label trend-goal-label" x="${width - margin.right}" y="${margin.top + 2}" text-anchor="end">Goal ${goal} lb is below this view</text>`;

  svg.innerHTML = `${grid}
    ${goalLine}
    <polygon class="trend-chart-area expanded" points="${areaPoints}"></polygon>
    ${rawPoints}
    <polyline class="trend-average-line" points="${trendPoints}"></polyline>
    ${dateLabels}`;
}

function renderTrendModalSummary(series) {
  const latest = latestWeightEntry();
  const trend = series.at(-1);
  const trendWeight = trend?.trendWeight ?? state.settings.startWeight;
  const change = trendWeight - state.settings.startWeight;
  $('expanded-trend-weight').textContent = `${trendWeight.toFixed(1)} lb`;
  $('expanded-latest-weight').textContent = latest ? `${latest.weight.toFixed(1)} lb` : '—';
  $('expanded-change-weight').textContent = `${change > 0 ? '+' : ''}${change.toFixed(1)} lb`;
  $('expanded-goal-label').textContent = `${state.settings.goalWeight} lb`;
  document.querySelectorAll('[data-trend-range]').forEach(button => {
    const active = button.dataset.trendRange === weightTrendRange;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', String(active));
  });
}

function renderWeightTrendViews() {
  const series = rollingWeightSeries(3);
  renderCompactTrendChart(series);
  renderTrendModalSummary(series);
  renderExpandedTrendChart(series);
}

function openWeightTrendModal() {
  const modal = $('weight-trend-modal');
  modal.classList.remove('hidden');
  document.body.classList.add('modal-open');
  renderWeightTrendViews();
  window.setTimeout(() => $('weight-trend-close').focus(), 0);
}

function closeWeightTrendModal() {
  $('weight-trend-modal').classList.add('hidden');
  document.body.classList.remove('modal-open');
  $('dashboard-trend-open').focus();
}
...
function renderDashboard() {
  ...
  const latest = latestWeightEntry();
  const trend = latestTrendPoint();
  const trendWeight = trend?.trendWeight ?? state.settings.startWeight;
  const progress = weightProgress(trendWeight);
  const lost = Math.max(0, state.settings.startWeight - trendWeight);
  const remaining = Math.max(0, trendWeight - state.settings.goalWeight);
  ...
  $('dashboard-trend-weight').textContent = trendWeight.toFixed(1);
  $('dashboard-trend-sample').textContent = trend
    ? `Based on ${trend.sampleSize} recorded day${trend.sampleSize === 1 ? '' : 's'} · updated ${formatDate(trend.date, false)}`
    : 'Starting baseline';
  $('dashboard-latest-weight').textContent = latest ? `${latest.weight.toFixed(1)} lb` : '—';
  $('dashboard-loss-total').textContent = `${lost.toFixed(1)} lb`;
  $('dashboard-goal-remaining').textContent = `${remaining.toFixed(1)} lb`;
...
function bindEvents() {
  ...
  $('dashboard-trend-open').addEventListener('click', openWeightTrendModal);
  $('weight-trend-close').addEventListener('click', closeWeightTrendModal);
  $('weight-trend-backdrop').addEventListener('click', closeWeightTrendModal);
  document.querySelectorAll('[data-trend-range]').forEach(button => button.addEventListener('click', () => {
    weightTrendRange = button.dataset.trendRange;
    renderWeightTrendViews();
  }));
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && !$('weight-trend-modal').classList.contains('hidden')) closeWeightTrendModal();
  });
